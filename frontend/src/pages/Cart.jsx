            ...
            {cart.length === 0 ? (
                ...
                <div className="product-details-container animate-fade-in" style={{display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '40px', margin: 0, padding: 0, background: 'transparent', border: 'none', boxShadow: 'none'}}>
                    <div className="cart-items">
                        {cart.map(item => (
                            <div key={item._id} style={{display: 'flex', gap: '20px', background: 'var(--surface-color)', padding: '20px', borderRadius: 'var(--radius-md)', marginBottom: '20px', border: '1px solid var(--border-color)', position: 'relative'}}>
                                <img src={item.image.startsWith('http') ? item.image : `http://localhost:5000/${item.image}`} alt={item.title} style={{width: '120px', height: '120px', objectFit: 'cover', borderRadius: 'var(--radius-sm)'}} onError={(e) => { e.target.onerror = null; e.target.src = 'http://localhost:5000/uploads/hero_image.png'; }} />
                                <div style={{flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between'}}>
                                    <div>
                                        <h3 style={{fontSize: '1.2rem', marginBottom: '8px'}}>{item.title}</h3>
                                        <div style={{color: 'var(--accent)', fontWeight: 'bold', fontSize: '1.2rem'}}>₹{item.price.toFixed(2)}</div>
                                    </div>
                                    <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                                        <div style={{display: 'flex', alignItems: 'center', background: 'var(--bg-color)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)', overflow: 'hidden'}}>
                                            <button onClick={() => updateQuantity(item._id, item.quantity - 1)} style={{background: 'none', border: 'none', borderRight: '1px solid var(--border-color)', color: 'var(--text-main)', padding: '8px 15px', cursor: 'pointer', fontSize: '1.1rem', transition: 'var(--transition)'}}>-</button>
                                            <span style={{padding: '0 15px', fontWeight: '500'}}>{item.quantity}</span>
                                            <button onClick={() => updateQuantity(item._id, item.quantity + 1)} style={{background: 'none', border: 'none', borderLeft: '1px solid var(--border-color)', color: 'var(--text-main)', padding: '8px 15px', cursor: 'pointer', fontSize: '1.1rem', transition: 'var(--transition)'}}>+</button>
                                        </div>
                                        <button onClick={() => removeFromCart(item._id)} style={{background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', textDecoration: 'underline', fontSize: '0.9rem'}}>Remove</button>
                                    </div>
                                </div>
                            </div>
                        ))}
